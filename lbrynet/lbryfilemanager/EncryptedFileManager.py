...
class EncryptedFileManager(object):
    ...
    def change_lbry_file_status(self, lbry_file, status):
        log.info("Changing status of %s (%s) to %s", lbry_file.rowid, lbry_file.stream_hash, status)
        result = yield self.storage.change_file_status(lbry_file.rowid, status)
    ...
    def delete_lbry_file(self, lbry_file):
        ...
        try:
            ...
            yield self.storage.delete_lbry_file_options(lbry_file.rowid)
